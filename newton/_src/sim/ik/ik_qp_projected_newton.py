...
"""Projected-Newton QP optimizer for differential inverse kinematics.
...
velocity limits.  Instead of ADMM, this solver uses projected Newton
iterations: each step performs a full Cholesky solve with gradient
masking for active bound constraints, followed by box projection.  For
small-DOF robots (<=7) this typically converges in 1-3 iterations from
a warm start and returns the *exact* QP solution.
"""
...
class IKOptimizerQPProjectedNewton:
    """Projected-Newton QP optimizer for batched differential inverse kinematics.
    ...
    limits).  The solver uses projected Newton iterations fused into a single
    tiled Warp kernel: each iteration computes the gradient, masks bound-active
    components, performs a Cholesky solve, and projects back onto the feasible
    box.  The previous solution is warm-started across outer IK iterations.
    ...
    Args:
        ...
        qp_pn_max_iters: Maximum projected-Newton iterations per QP solve.
        qp_tol: Convergence tolerance on the Newton step infinity norm.
    ...
    def __init__(
        ...
        *,
        ...
        self.qp_pn_max_iters = qp_pn_max_iters
        self.qp_tol = qp_tol
        ...
        self._alloc_pn_buffers(velocity_limit)
    ...
    def _alloc_pn_buffers(self, velocity_limit: np.ndarray | None) -> None:
        """Allocate projected-Newton warm-start and bound buffers."""
        device = self.device
        D = self.n_dofs
        B = self.n_batch

        # z_io stores the warm-start solution from the previous outer iteration
        self.pn_z = wp.zeros((B, D), dtype=wp.float32, device=device)
        self.pn_lb = wp.zeros((B, D), dtype=wp.float32, device=device)
        self.pn_ub = wp.zeros((B, D), dtype=wp.float32, device=device)
    ...
    # ------------------------------------------------------------------
    # Shared infrastructure (mirrors LM / QP optimizer)
    # ------------------------------------------------------------------
    ...
    def step(
        ...
        """Run several differential-IK outer iterations via projected-Newton QP.

        Each outer iteration relinearizes (FK + Jacobian), builds a QP, and
        solves it with projected Newton iterations to get a displacement
        ``dq``.  The previous solution is warm-started across iterations.
    ...
    def _step(self, joint_q: wp.array2d[wp.float32], step_size: float = 1.0) -> None:
        """One outer IK step: FK, Jacobian, projected-Newton QP, integrate."""
        ...
        # 4. Solve QP via fused projected-Newton kernel (warm-started)
        self.dq_dof.zero_()
        ...
        # 5. Integrate: q_new = q + step_size * dq
        wp.copy(self.dq_dof, self.pn_z)
        self._integrate_dq(
            joint_q,
            dq_in=self.dq_dof,
    ...
    def reset(self) -> None:
        """Clear warm-start state before a new solve sequence."""
        self.pn_z.zero_()
    ...
    # ------------------------------------------------------------------
    # Fused projected-Newton tiled kernel (overridden by specialized subclass)
    # ------------------------------------------------------------------
    ...
    @classmethod
    def _build_specialized(cls, key: tuple[int, int, str]) -> type[IKOptimizerQPProjectedNewton]:
        """Build a specialized subclass with a fused tiled projected-Newton kernel.

        The kernel runs the full projected-Newton loop (gradient computation,
        active-set masking, Cholesky solve, box projection, convergence check)
        in a single Warp ``launch_tiled`` with no CPU round-trips.  It is
        specialized per ``(n_dofs, n_residuals)`` so that tile dimensions are
        compile-time constants.
        """
        C, R, _ = key

        def _pn_fused_template(
            ...
            # Load Jacobian and residual tiles (constant across PN iters)
            J = wp.tile_load(jacobians[row], shape=(RES, DOF))
            r = wp.tile_load(residuals[row], shape=(RES, 1))

            # Precompute J^T and H = J^T J + damping * I
            Jt = wp.tile_transpose(J)
            JtJ = wp.tile_zeros(shape=(DOF, DOF), dtype=wp.float32)
            wp.tile_matmul(Jt, J, JtJ)

            # Add damping to diagonal: H = J^T J + damping * I
            damping_diag = wp.tile_zeros(shape=(DOF,), dtype=wp.float32)
            for i in range(DOF):
            ...
            for i in range(DOF):
                val = z_io[row, i]
                # Clamp warm-start to current bounds (bounds may have changed)
            ...
            # Write back solution and warm-start state
            wp.tile_store(dq_out[row], x)
            for i in range(DOF):
                z_io[row, i] = x[i]

        _pn_fused_template.__name__ = f"_qp_pn_fused_{C}_{R}"
        _pn_fused_template.__qualname__ = f"_qp_pn_fused_{C}_{R}"
        _qp_pn_fused_kernel = wp.kernel(enable_backward=False, module="unique")(_pn_fused_template)
        ...
        class _Specialized(IKOptimizerQPProjectedNewton):
            ...
            def _qp_solve_fused(self, jac, res, z, lb, ub, dq):
                wp.launch_tiled(
                    _qp_pn_fused_kernel,
                    dim=[self.n_batch],
                    inputs=[jac, res, z, lb, ub, self.damping, self.qp_tol, self.qp_pn_max_iters, dq],
                    block_dim=self.TILE_THREADS,
                    device=self.device,
                )

        _Specialized.__name__ = f"IKQPPN_{C}x{R}"
